		...
		initialize: function(data) {
			...
			this.ScaleClass = Chart.Element.extend({
				initialize: function(){
					...
					if (this.showLabels)
					{
						// Labels mean move the drawingArea to the right
						this.ctx.fillStyle = this.textColor;
						this.ctx.font = this.font;
						xMargin = this.ctx.measureText("0.0").width + 5;
						...
						yMargin = this.fontSize * dpi / 72; // font size is in pts which on cavas equals px
					}
					
					this.size = helpers.min([this.height - (2 * yMargin), this.width - (2 * xMargin)]);
					this.drawingArea = (this.display) ? (this.size / 2) - (this.fontSize / 2 + this.backdropPaddingY) : (this.size / 2);
					this.drawCenterX = (this.display) ? this.backdropPaddingX + xMargin + (this.drawingArea / 2) : xMargin + (this.drawingArea / 2);
					this.drawCenterY = (this.display) ? this.backdropPaddingY + yMargin + (this.drawingArea / 2) : yMargin + (this.drawingArea / 2);
				},
				...
				draw: function() {
					...
					if (this.display)
					{
						...
						helpers.each(rCircles, function(r, rIndex) {
							...
							if (this.showLabels) {
								// We draw labels at the left side of the circle but do not label 0
								if (Math.round(r * 10) % 2 === 0) {
									this.rLabels.push({
										text : r,
										x : x - radius,
										y : this.drawCenterY
